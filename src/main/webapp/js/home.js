...
function writetable(result) {
	...
	for (var i = 0; i < result[1].length; i++) {
		lastV += "<li style='height: 30'><div class='number_div'>" +
				(i+1)+"</div><span><a href='player.do?id="+result[1][i].data.videoId+"&src="+result[1][i].data.proName+"'>"+result[1][i].data.videoName+"</a></span></li>";
	}
	$('#lastVideo').html(lastV);
	var hotv = "";
//	alert(message)
	for (var i = 0; i < result[0].length; i++) {
		hotv += "<div class='video_file_div'><a href='player.do?id="+result[0][i].data.videoId+"&src="+result[0][i].data.proName+"'><img src='images/video/"+result[0][i].data.picName
		+"' /></a><div class='video_detail_div'>" +
				"<div class='video_title_div'>"+result[0][i].data.videoName+"</div><div class='video_author_div'>作者:<span>"
	...
	for (var i = 0; i < result[2].length; i++) {
		...
		+result[2][i].data.auName+"</span><span style='width: 25%;float: left;'>作者："+result[2][i].name+
	...
	for (var i = 0; i < result[3].length; i++) {
		lastau += "<li style='height: 30'><div class='number_div'>" +
				(i+1)+"</div><span><a href='language.do?id="+result[3][i].data.auId+"'>"+result[3][i].data.auName+"</a></span></li>";
	}
	$('#lastau').html(lastau);
	...
	for (var i = 0; i < result[5].length; i++) {
		lastpic += "<li style='height: 30'><div class='number_div'>" +
				(i+1)+"</div><span><a href='scenery.do?id="+result[5][i].data.picId+"&src="+result[5][i].data.proName+"'>"+result[5][i].data.picName+"</a></span></li>";
	}
	...
	for (var i = 0; i < result[4].length; i++) {
		...
		result[4][i].data.picName+"</span></div><a href='scenery.do?id="+result[4][i].data.picId+"&src="+result[4][i].data.proName+"' onmouseover='showDetail()' onmouseout='hiddenD" +
				"etail()'><img src='images/scenery/"+result[4][i].data.proName+"' /></a>" +
